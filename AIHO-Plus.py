...
def AIHO_Plus(lambda_rate, func_num, cost_diff, cpu_core, cache_capacity,
    ...
    for u, v, _ in G_cross_net.edges(data=True):
        if u in sub_1:
            ...
            bandwidth = bandwidth_mmWave_24
        if (u in sub_1 and v in sub_1) or (u in sub_6 and v in sub_6) or (
                u in mmWave_24 and v in mmWave_24):
    ...
    def TS_closest_soft_ddl(wait_queue, cur_time):
        return sorted(wait_queue,
                      key=lambda x: x.arvTime + x.softDdl - cur_time)
    ...
    print(lambda_rate, func_num, cost_diff, cpu_core, cache_capacity,
              soft_ddl_param, node_selection_strategy, path_selection_strategy,
              task_sorting_strategy, np.mean(list(delayDic.values())), sep=',')
